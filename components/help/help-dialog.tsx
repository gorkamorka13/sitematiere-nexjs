...
import { X, Shield, HelpCircle, User, Users, Info, FileText, Check, LayoutDashboard, Image as ImageIcon, Globe, Zap } from "lucide-react";
...
import { GammesJumbotron } from "./gammes-jumbotron";
...
export default function HelpDialog({ isOpen, onClose, user }: HelpDialogProps) {
  ...
  const [activePickerRange, setActivePickerRange] = useState<string | null>(null);
  ...
  const handleImageSelect = async (url: string) => {
    if (!activePickerRange) return;
    const key = `range_image_${activePickerRange}`;
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-10 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-700">
          ...
          {activeTab === "gammes" && (
            <GammesJumbotron 
              ...
              userRole={user.role}
              onImagePickerOpen={(rangeKey) => {
                setActivePickerRange(rangeKey);
