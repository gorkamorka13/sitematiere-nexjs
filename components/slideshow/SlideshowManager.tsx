    ...
    export function SlideshowManager({ projects }: SlideshowManagerProps) {
        ...
        const filteredProjects = useMemo(() => {
            return projects.filter(project => {
                ...
                if (selectedName && project.name !== selectedName) return false;
                return true;
            });
        ...
        // Auto-select project if single result
        useEffect(() => {
        ...
        const handleAddImage = async (url: string, name: string, fileId: string) => {
            const success = await addImage(url, name, fileId);
            if (success) setShowImagePicker(false);
        };
                                    ...
                                    <div className="space-y-3">
                                        {slideshowImages.map((slideshowImage, index) => (
                                            <SortableImageCard
                                                ...
                                                onRemove={() => removeImage(slideshowImage.id)}
                                            />
                                        ))}
                                    </div>
                                </SortableContext>
