...
export function SignOutButton({ variant = "default" }: SignOutButtonProps) {
    ...
    if (variant === "sidebar") {
        return (
            <button
                onClick={() => signOut({ callbackUrl: "/login" })}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold text-gray-500 hover:text-red-600 bg-gray-50/50 dark:bg-gray-900/20 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all group cursor-pointer"
                title="Déconnexion"
            >
