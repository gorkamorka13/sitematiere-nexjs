...
export function ProjectSelectDialog({ isOpen, onClose, onSelect }: ProjectSelectDialogProps) {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-md flex flex-col max-h-[80vh] border border-gray-100 dark:border-gray-800">
                <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
                    <h3 className="font-bold text-lg dark:text-white">Enregistrer dans la base</h3>
                ...
                <div className="p-4 overflow-y-auto flex-1">
                    <p className="text-sm text-gray-500 mb-4">Choisissez un projet pour enregistrer votre image modifiée.</p>
                    ...
                    <div className="space-y-1">
                        ...
                        {/* Option for global upload if supported by API */}
                        <button
                                    onClick={() => setSelectedId('global_unassigned')}
                                    className={`w-full flex items-center gap-3 p-3 rounded-lg text-left transition-all ${
                                        selectedId === 'global_unassigned'
                                        ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-200 dark:border-indigo-800 border'
                                        : 'hover:bg-gray-50 dark:hover:bg-gray-800 border-transparent border'
                                    }`}
                                >
