...
export default async function ExportDatabasePage({
  ...
  // Verification double : Session ADMIN + Mot de passe
  ...
  try {
    ...
    const [users, projects, files, slideshowImages] = await Promise.all([
      prisma.user.findMany({
        select: { id: true, email: true, name: true, username: true, role: true, color: true, createdAt: true, updatedAt: true },
      }),
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 md:p-8">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Utilisateurs", count: users.length, color: "text-blue-500" },
              { label: "Projets", count: projects.length, color: "text-green-500" },
              { label: "Fichiers", count: files.length, color: "text-orange-500" },
              { label: "Média", count: projects.reduce((acc, p) => acc + p.images.length + p.videos.length, 0), color: "text-purple-500" }
            ].map((stat, i) => (
